...
class FR3VRTeleopFull(Node):
    def __init__(self):
        ...
        self.declare_parameter('linear_scale', 1.9)       # 调大一点，配合死区使用
        ...
        self.get_logger().info("✅ 节点启动：已启用由参数控制的死区逻辑")

    # ================== 3. 核心运动逻辑 ==================
    def vr_callback(self, msg):
        # 夹爪逻辑：Grip 键切换
        current_grip = getattr(msg, 'grip_button', False)
        if current_grip != self.last_grip_state:
            self.control_gripper(close=current_grip)
            self.last_grip_state = current_grip

        # 运动逻辑：按下 Trigger 键开始
        if msg.trigger_button:
            if not self.is_controlling:
                self.get_logger().info("🎮 运动开始")
    ...
    def process_motion(self, msg):
        # === 1. 参数获取 ===
        # 建议把 angular_scale 调小，防止旋转抢占速度资源
        # 原来是 0.8，建议改为 0.2 或 0.3 试一试
        lin_scale = self.get_parameter('linear_scale').value
        ang_scale = 0.5  # <--- 硬编码测试，或者去 yaml 里把参数改小
        alpha = self.get_parameter('smoothing_factor').value
        
        dz_lin = self.get_parameter('deadzone_linear').value
        dz_ang = self.get_parameter('deadzone_angular').value

        # === 2. 位移处理  ===
        # VR: [x=右, y=上, z=后]
        vr_x = msg.rel_pose.transform.translation.x
        vr_y = msg.rel_pose.transform.translation.y
        vr_z = msg.rel_pose.transform.translation.z

        # 映射逻辑 (根据 Debug 脚本的结果)
        tx = vr_z * lin_scale  # VR前后 -> 机器人前后 (X)
        ty = vr_x * lin_scale  # VR左右 -> 机器人左右 (Y)
        tz = vr_y * lin_scale  # VR上下 -> 机器人上下 (Z)

        # 位移死区
        if np.sqrt(tx**2 + ty**2 + tz**2) < dz_lin:
            tx, ty, tz = 0.0, 0.0, 0.0

        # === 3. 旋转处理 (核心修改部分) ===
        try:
            # 获取 VR 的原始旋转向量 (Rotation Vector)
            q = [
            ...
            # 计算原始旋转向量 [rx_raw, ry_raw, rz_raw]
            # 这代表绕 VR 坐标系 X, Y, Z 轴的旋转速度
            rot_vec_raw = R.from_quat(q).as_rotvec()
            
            # ---【关键修复】坐标轴重映射 ---
            # 必须与位移逻辑完全一致！否则会出现“推手柄导致翻滚”的怪象
            # VR_Z轴旋转 -> 机器人_X轴旋转 (Roll)
            # VR_X轴旋转 -> 机器人_Y轴旋转 (Pitch)
            # VR_Y轴旋转 -> 机器人_Z轴旋转 (Yaw)
            
            target_rx = -rot_vec_raw[2] * ang_scale # index 2 is z
            target_ry = rot_vec_raw[0] * ang_scale # index 0 is x
            target_rz = rot_vec_raw[1] * ang_scale # index 1 is y

            # 旋转死区 (模长判断)
            if np.linalg.norm([target_rx, target_ry, target_rz]) < dz_ang:
                target_rx, target_ry, target_rz = 0.0, 0.0, 0.0

        except Exception as e:
            self.get_logger().warn(f"Rotation cal error: {e}")
            target_rx, target_ry, target_rz = 0.0, 0.0, 0.0
        ...
        # 旋转 (现在已启用，且映射正确)
        self.target_twist.twist.angular.x = self.target_twist.twist.angular.x * (1-alpha) + target_rx * alpha
        self.target_twist.twist.angular.y = self.target_twist.twist.angular.y * (1-alpha) + target_ry * alpha
        self.target_twist.twist.angular.z = self.target_twist.twist.angular.z * (1-alpha) + target_rz * alpha
    # ... (smooth_stop, publish_twist, control_gripper, try_activate_servo 保持不变) ...
    ...
    def control_gripper(self, close=True):
        ...
        goal.command.position = self.get_parameter('gripper_close_pos').value if close else self.get_parameter('gripper_open_pos').value
        goal.command.max_effort = self.get_parameter('gripper_force').value
...
class FR3VRTeleopFull(Node):
    def __init__(self):
        ...
        # ✅ 新增：夹爪连续控制（trackpad_y）
        self.declare_parameter('gripper_axis', 'trackpad_y')      # 用 trackpad_y
        ...
        self.declare_parameter('gripper_rate', 15.0)              # 限频 Hz
        ...
        self.get_logger().info("✅ 节点启动：使用 trackpad_y 进行夹爪连续控制")

    # ================== 3. 核心运动逻辑 ==================
    def vr_callback(self, msg):
        # ✅ 夹爪控制逻辑：使用 trackpad_y 连续控制（来自第二份代码）
        self.update_gripper_from_trackpad(msg)

        # 运动控制逻辑：按下 Trigger 键开始（保持原逻辑）
        if msg.trigger_button:
            if not self.is_controlling:
                self.get_logger().info("🎮 运动开始")
                self.is_controlling = True
...
class FR3VRTeleopFull(Node):
    def __init__(self):
        ...
        # 这里把你的 linear_scale / angular_scale 解释为“闭环增益”
        # Kp_pos：m/s per m   (1/s)
        # Kp_rot：rad/s per rad (1/s)
        self.declare_parameter('linear_scale', 1.9)       # -> Kp_pos
        self.declare_parameter('angular_scale', 0.2)      # -> Kp_rot
        ...
        self.get_logger().info("✅ Pose→Twist 闭环版本启动：Trigger按下时建立锚点，按住时跟随目标位姿")
    ...
    def process_motion_pose_servo(self, msg):
        ...
        # 你之前速度映射是：tx=vr_z, ty=vr_x, tz=vr_y
        # 这里对“位移增量”做同样映射
        dp_robot = np.array([
            dvr_p[2],   # x <- vr_z
            dvr_p[0],   # y <- vr_x
        ...
        # === 旋转映射（沿用你之前 rotvec 的轴映射方式）===
        # 先把 dvr_q 转成 rotvec_raw（VR坐标系）
        ...
        # === 死区：对“误差”做死区（这比对输入做死区更像真正Pose伺服）===
